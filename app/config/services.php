...
// MySQL Example:
// Support either:
// - host: "127.0.0.1" + optional port: 3306
// - host: "127.0.0.1:3306" (common in local configs)
$dbHost = (string)($config['database']['host'] ?? '127.0.0.1');
$dbPort = $config['database']['port'] ?? null;
if (strpos($dbHost, ':') !== false) {
	[$hostOnly, $portOnly] = explode(':', $dbHost, 2);
	$dbHost = $hostOnly;
	if ($dbPort === null && $portOnly !== '') {
		$dbPort = $portOnly;
	}
}

$dsn = 'mysql:host=' . $dbHost . ($dbPort !== null ? ';port=' . $dbPort : '') . ';dbname=' . $config['database']['dbname'] . ';charset=utf8mb4';
